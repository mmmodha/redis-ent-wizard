...
export function omitCreateInputDatabases(input: Record<string, unknown>): Record<string, unknown> {
  const clusters = Array.isArray(input.clusters) ? (input.clusters as Record<string, unknown>[]) : [];
  return {
    ...input,
    clusters: clusters.map((c) => {
      const { databases: _dropped, ...rest } = c;
      return rest;
    }),
  };
}

export function isTrialShardLicenseError(error?: string): boolean {
  const text = error || "";
  return /invalid_license/i.test(text) || /total shards count exceeds/i.test(text);
}
